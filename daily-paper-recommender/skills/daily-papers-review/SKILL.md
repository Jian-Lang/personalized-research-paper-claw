...
抓取阶段已经按 `daily_papers.conference_preferences.keywords` / `negative_keywords` 对 title + abstract 计分，并保证候选论文达到 `daily_papers.conference_preferences.min_score`。点评阶段不要再发明新的过滤规则；如果摘要确实显示与用户偏好很弱，可以放到“可跳过”，但不要因为没有全文就删除它。
...
- 🧭 **一句话总结**: 用一句中文概括这篇论文到底在做什么、主要价值或主要风险。必须基于 title + abstract，不超过 35 个中文字符；放在摘要上方，作为一眼判断入口。
...
- 🧭 **一句话总结**: 一句中文概括论文内容、价值或风险，不超过 35 个中文字符。
