...
先通过 `../_shared/user_config.py` 读取配置。`VAULT_PATH` 是根级 `config/user-config.local.json` 中的 Markdown 根目录；索引与 git 开关来自 Noter 专属配置，未配置字段回退到代码默认值。
...
运行 `ensure_manual_layout()`，确保 `{VAULT_PATH}/PersonalizedPaper/mocs` 和 `{VAULT_PATH}/PersonalizedPaper/papers` 存在。只初始化 Personalized 项目，不创建 Daily 或 Domain 目录。
