...
[🧩 核心功能](#features) · [🎯 推荐逻辑](#scoring) · [📦 安装](#installation) · [🚀 使用方式](#usage) · [🖼️ Domain Research Gallery 展示（完全由本 Skill 产生）](https://jianlang.org/html/mllm-personalized-understanding.html) · [🖥️ 项目交互式主页](https://jianlang.org/html/research-paper-claw/)
...
好不容易，您挑出了几篇，想着“明天认真读”。几天后，PDF 躺在下载目录，摘要留在某次对话里，零散判断写进临时 Markdown。等到需要整理 related work 时，您只记得“好像读过一篇”，却想不起它解决了什么、和现有工作有什么关系，更找不到当时为什么觉得它重要。

Personalized Research Paper Claw 就从这两个时刻开始工作：

- 📬 **Daily Conf Paper Delivery** 记住您的 topics、keywords 与排除项，从持续更新的顶会接收列表中挑出真正相关的论文，把摘要、命中原因和阅读优先级送到您面前。
- 📝 **Research Paper Noter** 接住您决定深入阅读的工作，生成详细笔记；在 Domain 模式下，它还会把论文组织成可浏览的 Research Gallery（支持一键导出 HTML，方便更灵活地分享），写清一项工作与前序、同期及同领域工作的关系。

一个负责让值得读的论文及时出现，一个负责让已经读过的内容不再消失。这里的 Personalized 不会自动揣测您的兴趣：推荐条件和分类结构均由您明确配置；长期知识源是写入全局笔记目录的普通 Markdown，始终由您自己保管，并可在需要分享时按需导出 HTML。我们强烈推荐使用 Obsidian 阅读和维护这些知识资产。

> Personalized Research Paper Claw 不负责自动写论文或自动跑实验。它专注于个性化论文发现、阅读和知识整理。
...
- [Daily Conf Paper Delivery 使用说明](daily-conf-paper-delivery/README.md)
- [Daily Conf Paper Delivery 架构](daily-conf-paper-delivery/ARCHITECTURE.md)
- [Research Paper Noter 使用说明](research-paper-noter/README.md)
