...
{如果论文有 intro / teaser / motivation / overview 图，在这里立即重复嵌入最适合代表论文问题设定的图；没有则省略本段，不写“无”。}

### Intro / Teaser 图

![Intro / Teaser Figure]({intro_or_teaser_figure_url_or_wikilink})

**说明**: {这张图展示论文的问题设定、核心动机或整体任务场景。}
...
{如果论文有 framework / architecture / method overview / pipeline 图，在这里立即重复嵌入最能说明方法结构的图；没有则省略本段，不写“无”。这张图后面仍然要在 `## 关键图表` 中按 Figure 编号完整出现。}

### Framework / Architecture 图

![Framework / Architecture Figure]({framework_or_architecture_figure_url_or_wikilink})

**说明**: {这张图展示方法整体框架、核心模块连接关系或推理流程。}
...
<!-- 只保留理解方法必要的关键公式；不要输出 arXiv HTML / LaTeXML 抽取的“HTML 公式索引”或编号数学片段列表 -->
