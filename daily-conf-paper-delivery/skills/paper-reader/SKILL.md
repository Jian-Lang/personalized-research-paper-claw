...
先通过 `../_shared/user_config.py` 读取配置。所有笔记路径都从仓库根目录 `config/user-config.local.json` 中的全局 Markdown 根目录派生；索引与 git 开关来自 Daily 专属配置，未配置字段回退到代码默认值。
...
year: 2025
venue: ICML 2026  # 优先使用调用方/论文元数据里的正式 venue；没有时才回退到 arXiv / OpenReview
...
**venue 硬规则**：
- 如果当前调用来自 `daily-papers-notes` / 会议推荐链路，并且输入里已有 `venue` / `conference` / `source`，frontmatter 的 `venue` **必须**沿用该会议信息
- `arXiv` / `OpenReview` / `PDF` 只是阅读来源，不是发表 venue；不要因为你读的是 arXiv HTML 就把会议论文写成 `venue: arXiv`
- 只有在确实拿不到正式会议名时，才允许回退到 `arXiv`、`OpenReview` 或 `Unknown`
