...
from user_config import domain_project_content_dir, domain_project_papers_dir, domain_vault_path
...
def render_entry_block(domain: str, entry: dict[str, Any], index: int) -> list[str]:
    ...
    lines = [
        ...
        f"- **笔记**: [[{domain}/paper/{entry['note']}|{entry['note']}]]",
...
def render_entry_line(domain: str, entry: dict[str, Any]) -> str:
    link = f"{domain}/paper/{entry['note']}"
...
def update_domain_index(domain: str, content_dir: Path) -> Path:
    ...
    if page_paths:
        for path in page_paths:
            ...
            display = rel.replace("/", " / ")
            lines.append(f"- [[{domain}/content/{rel}|{display}]]")
    else:
        lines.append("- 暂无内容")
    lines.append("")
